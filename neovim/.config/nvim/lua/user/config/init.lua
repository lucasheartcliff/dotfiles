...
local defaults = {
  ...
  defaults = {
    autocmds = true, -- user.config.autocmds
    keymaps = true,  -- user.config.keymaps
    -- user.config.options can't be configured here since that's loaded before lazyvim setup
    -- if you want to disable loading options, add `package.loaded["user.config.options"] = true` to the top of your init.lua
  },
...
function M.init()
  if not M.did_init then
    ...
    require("user.util").lazy_notify()

    -- load options here, before lazy init while sourcing plugin modules
    -- this is needed to make sure options will be correctly applied
    -- after installing missing plugins
    require("user.config").load("options")
